...
const log: Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'GigService Cache', 'debug');
...
const getUserSelectedGigCategory = async (key: string): Promise<string> => {
  try {
    ...
    log.log('error', 'GigService Cache getUserSelectedGigCategory() method error:', error);
    return '';
  }
};
